% Methode de Cooper (avec toute les améliorations)
...
function detector(image, windowSize, baseThreshold)
    ...
    [Gx, Gy] = gradient(image);% Utile pour la direction
    
    % Gestion du seuil
    scalingFactor = (windowSize^2) / (3^2); % Basé sur du 3x3
    threshold = baseThreshold * scalingFactor;
    sigma_noise = std(image(:)); % Calcule l'écart-type du bruit de l'image
    
    dissimilarityThresholds = linspace(threshold / windowSize^2, threshold, windowSize^2);
    similarityThresholds = linspace(threshold / (2 * sigma_noise), threshold / sigma_noise, windowSize^2);
        ...
        for j = (1 + halfWindow):(cols - halfWindow)
            if contours(i, j) == 1
                ...
                for k = 1:numel(patch1)
                    ...
                    % Non similaire (coin)
                    if diffSum > dissimilarityThresholds(k)
                        jumpOut = true;
                        break;
                    end
                    
                    % Similaire (pas coin)
                    if diffSum < similarityThresholds(k)
    ...
    figure;
    imshow(contours);
    title('Contours détectés avec l''algorithme Canny');
    ...
    title('Coins détectés avec Early Jump-Out et cooper');
    hold off;

    % Affichage des seuils 
    figure;
    plot(dissimilarityThresholds, 'r-', 'LineWidth', 1.5);
    hold on;
    plot(similarityThresholds, 'b-', 'LineWidth', 1.5);
    xlabel('Indice de l''élément dans la fenêtre');
    ylabel('Valeur du seuil');
    legend('Seuil de dissimilarité', 'Seuil de similarité');
    title('Courbes des seuils de dissimilarité et de similarité');
    grid on;
    hold off;
